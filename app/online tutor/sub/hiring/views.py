...
def home(request):
    ...
    if request.user.is_superuser:
        ...
        tutorpost=TutorPost.objects.filter(admin_approved=True)
...
def requirements(request):
    if request.user.is_authenticated and not request.user.is_superuser:
        requirement=studentpost.objects.filter(admin_approved=True)
        context={
...
def admin_visit_tutorpost(request):
    if request.user.is_superuser:
        
        pending_posts=TutorPost.objects.filter(admin_approved=False)
        context={
            'pending_posts':pending_posts
        }
        return render(request, 'admin_tutorpost.html', context)
        
def approve_post(request, post_id):
    if request.user.is_superuser:
        approvepost=TutorPost.objects.get(id=post_id)
        approvepost.admin_approved=True 
        approvepost.save()
        return redirect('admin_visit_tutorpost')

def admin_visit_studentpost(request):
    if request.user.is_superuser:
        
        pending_posts=studentpost.objects.filter(admin_approved=False)
        context={
            'pending_posts':pending_posts
        }
        return render(request, 'admin_studentpost.html', context)

def approve_studentpost(request, post_id):
    if request.user.is_superuser:
        approvepost=studentpost.objects.get(id=post_id)
        approvepost.admin_approved=True 
        approvepost.save()
        return redirect('admin_visit_studentpost')